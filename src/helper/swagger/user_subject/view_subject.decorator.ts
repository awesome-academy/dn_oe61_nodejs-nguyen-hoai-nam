import { applyDecorators } from "@nestjs/common";
import { ApiOperation, ApiParam, ApiResponse } from "@nestjs/swagger";
import { ErrorResponseDto } from "src/validation/api_document/error/error.validation";
import { ServerErrorResponseDto } from "src/validation/api_document/error/server_error.validation";
import { ViewSubjectSuccessDto } from "src/validation/api_document/user_subject/view_subject_success";

export function ApiResponseViewSubject() {
    return applyDecorators(
        ApiOperation({
            summary: "Xem chi tiết 1 môn học và các nhiệm vụ thuộc bài học",
            description: "Cho phép người dùng xem chi tiết 1 môn học và các nhiệm vụ thuộc khoá học đó"
        }),

        ApiParam({
            name: 'userSubjectId',
            description: 'Mã môn học',
            type: Number,
            example: '2',
            required: true
        }),

        ApiResponse({
            status: 200,
            description: 'Thành công',
            type: ViewSubjectSuccessDto
        }),

        ApiResponse({
            status: 400,
            description:
                'Lỗi xác thực. Các lỗi khác cũng trả về định dạng giống, chỉ thay đổi `status`, `message`, `code`, `path`',
            type: ErrorResponseDto,
        }),

        ApiResponse({
            status: 500,
            description: 'Lỗi hệ thống, có thể do server hoặc cơ sở dữ liệu',
            type: ServerErrorResponseDto,
        }),
    )
}
