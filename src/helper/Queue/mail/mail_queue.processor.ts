...
export class MailQueueProcessor {
  ...
  async handleReminderEmail(job: Job<MailJobData>) {
    const { to, subject, template, context } = job.data;

    if (!to || !subject || !template) {
      this.logger.warn(`Missing required email fields. Job data: ${JSON.stringify(job.data)}`);
      return;
    }
    ...
    try {
      ...
      this.logger.error(`Failed to send email to ${to}`, error.stack, 'MailQueueProcessor.handleReminderEmail',);
    }
  }
}
