document.addEventListener('DOMContentLoaded', () => {
    ...
    const buildSubjectRow = (s) => {
        const formatDate = (dateString) => dateString ? new Date(dateString).toLocaleDateString() : tMsg('common.notAvailable');
    ...
    const populateTraineeInfo = (trainee) => {
        ...
        if (el('trainee-name')) el('trainee-name').textContent = trainee.userName || trainee.name || tMsg('common.notAvailable');
        if (el('trainee-email')) el('trainee-email').textContent = trainee.email || tMsg('common.notAvailable');
    };
