document.addEventListener('DOMContentLoaded', function () {
  ...
  async function fetchProfile() {
    ...
    try {
      ...
      userRoleDisplay.textContent = roleValue;
      userRoleDisplay.className = `px-3 py-1 text-xs font-semibold text-white rounded-full ${roleClasses[roleValue] || 'bg-gray-500'}`;

      userStatusDisplay.textContent = statusValue;
      userStatusDisplay.className = `px-3 py-1 text-xs font-semibold text-white rounded-full ${statusClasses[statusValue] || 'bg-gray-500'}`;
