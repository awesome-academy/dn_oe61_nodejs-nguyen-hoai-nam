document.addEventListener('DOMContentLoaded', () => {
    ...
    const populateUI = (data) => {
        ...
        document.getElementById('trainee-name-subheader').textContent = `For Trainee: ${user.userName}`;
        ...
        document.getElementById('started-at-date').textContent = startedAt ? new Date(startedAt).toLocaleDateString() : 'Not Started';
        ...
        if (status === 'NOT_STARTED') {
            ...
            startButton.innerHTML = '<i class="fas fa-play mr-2"></i> Start Subject';
            ...
            userTasks.forEach(userTask => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="#" class="text-indigo-600 hover:text-indigo-900">View Task</a>
                    </td>
                `;
                taskListBody.appendChild(row);
            });
        } else {
            taskListBody.innerHTML = '<tr><td colspan="3" class="px-6 py-4 text-center text-gray-500">No tasks found for this subject.</td></tr>';
        }
    };
    ...
    const handleStartSubject = async (id) => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to start subject.');
            }
